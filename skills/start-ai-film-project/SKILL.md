...
description: 为小陌 AI 影视工作流创建全新递增版本项目并校验模板。按项目模板标识路由 v0.8 五岗位启动或 v0.9/V1 六部门入口；工作室模式不预建待命任务。用户要求新建项目、创建项目文件夹或从模板开始时使用，不用于继续、迁移已有项目或执行媒体动作。
---
...
V1 的创建、保存、恢复与六部门分工沿用下方工作室分支，JSON 的 workflowVersion 和 launchPrompt 明确为 1.0。图片生成/编辑只在用户本次明确授权范围内，按共同契约“V1 原生图片工具”使用当前可用 Codex 原生工具，默认选择原生。media_execution=false 仍只约束文本运行时；建档本身不授权生图，视频、上传与发布权限不扩张。
...
V0.9：保留六个根 Markdown（开始这里及 A/B/C/D/E），另建能力文件与 C1/C2 工作稿入口。JSON 返回六部门短启动词、哈希与一个 launchPrompt；能力 stage=active 才交付活动入口，其他阶段保留隔离候选范围。交付 launchPrompt，用户在新项目使用；首个任务按 A 统筹入口开始，按需进入专业。不创建六个待命任务，不要求补齐任务拓扑，不调用旧同岗缓存。只有用户明确要求新开/续接任务才调用任务工具；运行时开启且继续同一项目内同一工作集时，才通过 PrepareHandoff/ActivateHandoff/CancelHandoff 办理接管，能力关闭时不冒称自动恢复。
...
成功后给出 JSON 返回的项目绝对路径及完整 launchPrompt，不凭记忆重写或持久化绝对路径。角色原始启动词保存在项目入口，V0.9 的 departmentPrompts 便于用户按需选择。
